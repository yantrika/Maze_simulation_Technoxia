...
#define WALL_THRESHOLD 0.100   // meters (13 cm)
#define FRONT_THRESHOLD 0.110  // meters (7 cm)

// Robot dimensions
#define WHEEL_RADIUS 0.03   // 3 cm
#define AXLE_LENGTH 0.18   // 16 cm

double prev_left_distance = 0.0;  // remembers last valid left wall distance
...
int offset = .1;   // wall-following offset
...
int main() {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    if (!leftWall && !rightWall && !frontWall){
      ...
      double target_distance = 0.1; // 20 cm
      ...
      double turn_rotation = turn_arc / WHEEL_RADIUS;
        ...
        while (wb_robot_step(TIME_STEP) != -1) {
          ...
          if (fabs(right_pos) >= turn_rotation+0.6
          ) {
            wb_motor_set_velocity(left_motor, 0.0);
            wb_motor_set_velocity(right_motor, 0.0);
          ...
          // ---------- STEP 1: Reset encoders ----------
        left_start = wb_position_sensor_get_value(left_sensor);
        right_start = wb_position_sensor_get_value(right_sensor);
      ...
      // Move forward until 0.2 m distance
      target_distance = 0.1; // 20 cm
      target_rotation = target_distance / WHEEL_RADIUS;
      ...
      // Move forward until 0.2 m distance
      double target_distance = 0.2; // 20 cm
      double target_rotation = target_distance / WHEEL_RADIUS;
        ...
        while (wb_robot_step(TIME_STEP) != -1) {
    ...
    else { //followLeft = 0; // switch to right
          ...
          while (wb_robot_step(TIME_STEP) != -1) {
            ...
            if (fabs(left_pos) >= turn_rotation) {
              wb_motor_set_velocity(left_motor, 0.0);
              wb_motor_set_velocity(right_motor, 0.0);
              printf("Completed 90 degree right turn!\n");
              break;
            }
          }
         }
  ...
  // Update stored left wall distance only if wall exists
    if (leftWall && !left_Wall1) {
        prev_left_distance = left_dist;
        left_Wall1=true;
    }
    // Straight ahead – ignore right PID, maintain last left wall distance
    //double error = prev_left_distance - wb_distance_sensor_get_value(ds_left);
    //double correction = Kp * error;  // simple proportional correction
    
    //double base_speed = 3.0;
    //wb_motor_set_velocity(left_motor, base_speed + correction );
    //wb_motor_set_velocity(right_motor, base_speed - correction);

    //printf("Going straight, maintaining previous left wall distance.\n");
}
        ...
        // Go RIGHT only if forced (front blocked, left blocked)
    ...
    printf("%d\t%d\t%d\n",leftWall,rightWall,frontWall);
    if (leftWall && !frontWall && !rightWall)
    printf("going straight\n");
     // no left but straight also right avail
